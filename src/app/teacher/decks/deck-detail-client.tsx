...
import {
  ...
  getDictionaryMeaningCandidatesAction,
  importWordsAction,
  selectDictionaryMeaningAction,
  updateWordAction,
} from "./actions";
...
function WordRow({
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap gap-2">
        <span className="inline-flex w-fit rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-700 ring-1 ring-slate-200">
          {sourceLabel(word.dataSource)}
        </span>
        <span
          className={cn(
      ...
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-white/80 bg-white p-6 shadow-2xl shadow-slate-950/20">
        ...
        {!isConfirm ? (
          ...
          <div className="mt-6 rounded-2xl border border-indigo-100 bg-indigo-50 px-4 py-3 text-sm font-medium leading-6 text-indigo-800">
            Cache birinchi tekshiriladi, keyin dictionaryapi.dev ishlatiladi.
...
function WordModal({
  ...
  const [meaningSource, setMeaningSource] = useState<"CACHE" | "DICTIONARY">(
    "CACHE",
  );
  ...
  function loadMeanings(bypassCache: boolean) {
    if (!isEdit) {
      return;
    }

    setMeaningsOpen(true);
    setMeaningError("");
    startTransition(async () => {
      const result = await getDictionaryMeaningCandidatesAction({
        wordId: modal.word.id,
        bypassCache,
      });

      setMeaningSource(result.source);
      setMeaningCandidates(result.candidates);
      if (!result.ok) {
        setMeaningError(result.error ?? "Dictionary meanings topilmadi.");
      }
    });
  }

  function selectMeaning(candidate: MeaningCandidate) {
    if (!isEdit) {
      return;
    }
    ...
    setMeaningError("");
    startTransition(async () => {
      const result = await selectDictionaryMeaningAction({
        wordId: modal.word.id,
        candidate,
        ...
        <form action={action} className="space-y-5 p-6">
          ...
          {draft.lookupStatus === "NEEDS_REVIEW" ? (
            <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-800">
              So'z yoki yaxshi example topilmadi. Ma'noni qo'lda tekshiring yoki manual Gemini fallback ishlating.
            </div>
          ) : null}
          ...
          {meaningsOpen ? (
            <div className="rounded-2xl border border-indigo-200 bg-indigo-50/60 p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs font-medium text-slate-600">
                    Source: {sourceLabel(meaningSource)}. Kerakli ma'noni tanlang.
                  </p>
                </div>
                <Button
                  type="button"
              ...
              {isPending && meaningCandidates.length === 0 ? (
                <p className="mt-4 text-sm font-bold text-indigo-700">
                  Meanings yuklanmoqda...
                </p>
              ) : null}

              {meaningCandidates.length > 0 ? (
                <div className="mt-4 max-h-96 space-y-3 overflow-y-auto pr-1">
                  {meaningCandidates.map((candidate) => (
                    <button
                      key={candidate.id}
                      type="button"
                      onClick={() => selectMeaning(candidate)}
                      disabled={isPending}
                      className="block w-full rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm transition hover:border-indigo-300 hover:shadow-md disabled:opacity-60"
                    >
                      ...
                      <p className="mt-2 text-sm leading-6 text-slate-600">
                        {candidate.example || "Example mavjud emas — needs review."}
                      </p>
                      {candidate.pronunciationText ? (
                        <p className="mt-2 text-xs font-bold text-slate-500">
                          Pronunciation: {candidate.pronunciationText}
                        </p>
                      ) : null}
                    </button>
                  ))}
          ...
          <div className="block space-y-2">
            ...
            <span className="flex flex-wrap gap-2">
              ...
              {draft.lookupStatus ? (
                ...
                >
                  {draft.lookupStatus.replaceAll("_", " ")}
                </span>
              ) : null}
            </span>
          </div>
